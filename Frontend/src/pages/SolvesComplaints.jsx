  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      try {
        ...
        const res = await axios.get(`${API_BASE_URL}/api/resolved-reports`, {
      ...
      <div className="pt-10 pb-20 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto w-full">
        ...
        <div className="space-y-4 mb-8">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-slate-950 text-slate-900 dark:text-white placeholder-slate-400 text-sm pl-11 pr-4 py-3.5 rounded-2xl border border-slate-200 dark:border-slate-700/80 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all shadow-xs"
            />
          </div>
            ...
            {[
              ...
              >
                ...
                <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${
                  filter === t.id ? "bg-white/20 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400"
                }`}>
                  {t.count}
                </span>
              </button>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center rounded-3xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 max-w-md mx-auto space-y-3 shadow-sm">
            ...
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">No Archived Records</h3>
            <p className="text-xs text-slate-600 dark:text-slate-400">No records found matching current filters.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filtered.map((report) => {
              ...
              return (
                <div
                  ...
                  className={`p-6 sm:p-7 rounded-3xl bg-white dark:bg-slate-900/90 border shadow-sm dark:shadow-xl space-y-4 flex flex-col justify-between transition-all ${
                ...
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className={`px-2.5 py-0.5 rounded-lg text-xs font-semibold border flex items-center gap-1.5 ${
                        isResolved ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-500/30" : "bg-red-500/10 text-red-700 dark:text-red-300 border-red-500/30"
                      }`}>
                    ...
                    <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 line-clamp-2 leading-relaxed">{report.description}</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <div className="space-y-1">
                      <span className="text-[11px] font-semibold text-slate-600 dark:text-slate-400">Reported Photo</span>
                      <div className="h-28 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                        {report.image_url || report.imageUrl ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-[10px] text-slate-400 dark:text-slate-600">No Photo</div>
                        )}
                      </div>
                    </div>

                    <div className="space-y-1">
                      <span className={`text-[11px] font-semibold ${isResolved ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
                      ...
                      <div className="h-28 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                        {(report.admin_image_url || report.adminImageUrl) ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-[10px] text-slate-400 dark:text-slate-600">
                  ...
                  {(report.admin_remarks || report.adminRemarks) && (
                    <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300">
                      <span className="font-semibold text-slate-900 dark:text-slate-200">Remarks: </span>
